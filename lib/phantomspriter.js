...
page.evaluate(function () {
    var testdraw = function () {
        ...
        for (var i = 0; i < document.images.length; i++) {
            ...
            images.push({ height: image.height, width: image.width });
        }
        ...
        for (var j = 0; j < document.images.length; j++) {
            ...
            images[j].offset = allHeight;
            ctx.drawImage(document.images[j], 0, allHeight);
        }
        return JSON.stringify({image: canvas.toDataURL(), images: images, maxheight: maxheight, maxwidth: maxwidth}) + '<<<<ENDIMAGE';
    };
    setTimeout(function () {
